...
FluScrollablePage {
    ...
    Flickable {
        anchors.fill: parent; contentHeight: col.implicitHeight + 20; clip: true
        ColumnLayout {
            id: col; Layout.fillWidth: true; spacing: 16

            FluGroupBox { title: "下载设置"; Layout.fillWidth: true
                ColumnLayout { spacing: 8
                    FluButton { text: "下载目录: " + (backend.downloadPath||"未设置"); Layout.fillWidth: true }
                    FluToggleSwitch { text: "每次询问下载位置"; checked: backend.askDownloadLocation; onCheckedChanged: backend.askDownloadLocation=checked }
